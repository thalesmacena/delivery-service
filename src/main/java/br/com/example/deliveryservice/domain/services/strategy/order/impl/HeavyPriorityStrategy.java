...
public class HeavyPriorityStrategy implements OrderStrategy {
    ...
    public Order handleOrder(OrderPayload orderPayload, List<OrderProductDTO> orderProductList) {
        ...
        LocalDateTime calculateDeliveryTime = calculateDeliveryTime(orderProductList.size());
    ...
    private Long calculatePriorityTotalValueInCents(List<OrderProductDTO> orderProductList) {
        ...
        return Long.sum(partialTotal, 10000L);
    }

    private LocalDateTime calculateDeliveryTime(int quantity) {
        return LocalDateTime.now().plusWeeks(quantity);
    }

    @Override
    public ProductType getOrderPriorityStrategy() {
        return ProductType.HEAVY;
    }
}
